...
* See OSC test notes in OSC section below.
...
Do we want to parse all OSCs?  Originally resistant to it due to potential
variability in what's considered and OSC.  E.g. do we support closing with BELL
(xterm, osx term, and iterm2 all do)?

More importantly, tested terminals seem to require pattern "ESC]num;" as the
initiator, but the spec doesn't say anything about the "num;" part.  The
implementations just seem to consume the first char past the ] of OSC and then
give up (terminal, iterm2, xterm).

We'll support the set of OSC that start with 'num;' and don't contain any C0s.

It's also possible that we need multiple instances of the buffers alive
simultaneously as we could have a _prev_, _bound_, etc. all active.  We could
have one big buffer, but then we'd need to track when to release stuff.

Instead, will just retain indices into the original string.  What if a link is
split across two string elements?  Technically nl is allowed in OSC.  So if
there is a string with `carry=TRUE` it is possible that the OSC gets split
across two lines.  We're going to require that in order for OSC to be considered
valid they much be complete within a line (it's not even clear that an NL inside
an OSC is a NL anyway).
...
* Tests:
    * more parameters
    * colons and semicolons in bad places
    * id actually captured (and re-output)?
    * wrapping with plain oscs and urls
