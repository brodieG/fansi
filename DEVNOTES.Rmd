...
* Make sure `strwrap` and others consume initial SGR (substr does so)?  It looks
  like stwrap doesn't: `strwrap_ctl(c("\033[31mhello world", "\033[42mgoodbye moon"), carry=TRUE, 10)`
...
If we have both spans and anchors going, we need to:

* Always open/close both anytime state of either changes if state was active.
* Always open link first span second? (and close LIFO?).

The biggest remaining issue here is that it makes most sense to combine url and
sgr into the `_sgr` functions, but if we do, we're committed.  We will not be
able to in the future to go back and decide we want a `_url`.  A possible option
is to add `_state` that treat as special both URL and SGR.
...
And we have the `_sgr` flavors of existing functions.  Is there really any value
in the `_sgr` functions?  Pretty questionable.  That's pretty much the major
question.  Do we just want to retire the `_sgr` functions?  The best reason to
have them in would be if the `_ctl` functions did not warn when they encounter
things that could affect display, but they do.

So, likely solution is to retire the functions, rename all the new `_sgr`
functions `_state`, and leave it at that.

What if a link is split across two string elements?  Technically nl is allowed
in OSC.  So if there is a string with `carry=TRUE` it is possible that the OSC
gets split across two lines.  We're going to require that in order for OSC to be
considered valid they much be complete within a line (it's not even clear that
an NL inside an OSC is a NL anyway).

Behavior of C0 in OSC is not consistent on mac terminal.  C0s contained
therein are emitted.  Quartz xterm and iterm2 seem to support them properly.

Todo:

* Figure out if all the stuff like carry/terminate applies equally?
    * It should; every special handling we have for each of those we'll have to
      do for URLs too.
    * What a PITA, probably no way to merge and use the same logic?  Well, some
      of them we can.  E.g. close_sgr could be come close_state or some such.
* HTML translation tests (note id shouldn't carry as semantics are different in
  HTML vs OSC since ids may be duplicated in the latter).
