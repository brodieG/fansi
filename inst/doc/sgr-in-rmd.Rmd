...
### Browsers Do Not Interpret ANSI CSI SGR
...
same time the advent and increased adoption of JJ Alaire and Yihui Xie
`rmarkdown` and `knitr` packages it also became easy to automatically
incorporate R code and output in HTML documents.

Unfortunately these ANSI CSI SGR sequences are not recognized by web browsers,
and instead end up rendering weirdly<a href=#f1><sup>1</sub></a>:
...
markup if it think it is not outputting to a terminal.
