...
```{r echo=FALSE, child='./chunks/helper-funs.Rmd'}
```
...
In our case we prefer to only handle output that contains ANSI CSI SGR, and let
`knitr` do its thing the rest of the time.  The best way we have found to do
this is unfortunately a little awkward:

```{r eval=TRUE, child='./chunks/knit-hook.Rmd'}
```

The helper functions `html_esc` and `code_block` are defined in the
[appendix](#appendix).
...
```{r child='./chunks/helper-funs.Rmd', echo=TRUE, eval=TRUE}
```

blay blah blah

```{css}
PRE.r-fansi-output SPAN {
  padding-top: .25em;
  padding-bottom: .25em;
}
```
