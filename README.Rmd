...
`substr_ctl<-` will likely be a bit slower as it is implemented as several
`substr_ctl` operations, and in general operations involving `type = "width"`
will be a bit slower.  We have prioritized convenience and safety over raw
speed in the C code.
...
f <- tempfile()
writeLines(c("<html><pre>", to_html(N), "</pre></html>"), f)
browseURL(f)
```
