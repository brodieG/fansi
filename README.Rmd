...
is wrong as the naive string extraction loses the initial`"\033[37m"` that sets
...
manipulation `fansi` should be fast enough to keep attention away from itself.

## Width Based Substrings

`fansi` also includes improved versions of some of those functions, such as
`substr2_ctl` which allows for width based substrings:
...
`fansi` width calculations account for graphemes, including combining emoji:
...
browseURL(f)
```
![Translate to HTML](https://raw.githubusercontent.com/brodieG/fansi/readme/extra/images/sgr_to_html.png)
...
intended.  See the vignette for details.
